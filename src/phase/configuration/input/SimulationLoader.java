...
public class SimulationLoader {
	...
	public enum HistoryMode{CREATE,READ}
	...
	public SimulationLoader(String configFilePath) {
		...
		rootDirPath = config.getProperty(IConfig.PROPERTY_SIMULATION_LOADER_ROOT_DIRECTORY);
	}
	...
	public GlobalTimer load() throws IOException, ClassNotFoundException{
		...
		/*	
			...
			*/
			...
			faulTimer = buildFaultTimer(vehicles.size(),faultDescriptions,sensors,maxTimeTicks,config);
	...
	protected OutputTimer buildOutputTimer(List<Algorithm> algorithms, IConfig config) {
		...
		String inputConfigFilePath=config.getConfigFilePath();
		 ...
		 return new OutputTimer(null, experimentId,
				 ...
				 outputRocCurveImageFileName,inputFilesDirectory, histOutputFileName,inputBatchScript,allAlgorithms);
	}
	...
	protected static  PerformanceAnalysisTimer buildPerformanceAnalysisTimer(List<Algorithm> algorithms, IConfig config) {
		...
		return new PerformanceAnalysisTimer(algorithms,leftWindowSize,rightWindowSize,thresholdNumberDecimalPrecision);
	}
...
*/
	...
	protected static  List<Sensor> loadSensors(IConfig config) {
		...
		List<String> unparsedSensors = config.getProperties(IConfig.PROPERTY_SENSOR_WHITE_LIST);
	...
	protected static  List<Algorithm> loadAlgorithms(List<Vehicle> vehicles,IConfig config) {
		...
		//get the distannce metrix strings (example: "Helligner") for each algorithm
		...
		for(int i = 0;i<numAlgorithms;i++){
			...
			//alg.setName(distanceMetric.toString());
	...
	protected  FaultTimer buildFaultTimer(int numberVehicles,List<FaultDescription> faultDescriptions,List<Sensor> allSensors,int maxTimeTicks, IConfig config) throws InvalidPropertiesFormatException, IOException {
		Logger log = LoggerFactory.getInstance();
		...
		Noise faultProbGenerator =buildNoise(c,IConfig.PROPERTY_FAULT_FAULT_GENERATION_NOISE_CONFIG_FILE_PATH);
		Noise repairProbGenerator =buildNoise(c,IConfig.PROPERTY_FAULT_REPAIR_GENERATION_NOISE_CONFIG_FILE_PATH);
		List<FaultDescription> singleSensorFaultDescriptions =buildSingleSensorFaultDescriptions(allSensors,config);
