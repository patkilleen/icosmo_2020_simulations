...
createROCCurve <- function(output_csv,output_roc_curve_file){
	...
	mycolors =c('green','red','black','yellow','orange','blue')
	mysymboles = c('+','+','+','*','*','*')
	...
	barplot(areasUnderCurveVec[algNames], main="Area Under ROC Curve", horiz=FALSE, ylim=c(0,1.1), xlab="Algorithm", col = mycolors[algNames])
...
#http://www.endmemo.com/program/R/pchsymbols.php 
#https://www.statmethods.net/graphs/line.html
computeAreaUnderCurveCSV <- function(input_csv,output_auc_csv){

	print(paste0("reading output file: ",input_csv))
	outputData = readOutputData(input_csv)
	outputData = outputData[order(outputData$threshold),]
	#outputData = outputData[order(outputData$algorithm_name),]
	outputData = outputData[order(outputData$algorithm_id),]
	...
	write.csv(outputDataFrame, file = output_auc_csv,row.names=FALSE, na="")
	print(paste0("area under curve csv output: ",output_auc_csv))
}
...
computeAreaUnderCurveCSV (output_csv,paste0(output_roc_curve_file,".csv"))